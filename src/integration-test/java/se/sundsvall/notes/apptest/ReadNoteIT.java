...
class ReadNoteIT extends AbstractAppTest {
	...
	void test01_readById() throws Exception {
		setupCall()
			.withServicePath("/" + MUNICIPALITY_ID + "/notes/134aedef-2d33-410b-8654-207e9644fc3d")
	...
	void test02_readAllFilterByPartyIdContextRoleClientId() throws Exception {
		setupCall()
			.withServicePath("/" + MUNICIPALITY_ID + "/notes?partyId=a37b06cc-edda-459d-860c-9f8cd1e24b00&context=context1&role=role1&clientId=clientId1")
			.withHttpMethod(HttpMethod.GET)
			.withExpectedResponseStatus(HttpStatus.OK)
			.withExpectedResponse("response.json")
			.sendRequestAndVerifyResponse();
	...
	void test03_readByIdNotFound() throws Exception {
		setupCall()
			.withServicePath("/" + MUNICIPALITY_ID + "/notes/14c8fd87-2a57-4e38-bcd5-6fff9ca6880e") // Id does not exist in DB.
			.withHttpMethod(HttpMethod.GET)
	...
	void test04_readAllNoneFound() throws Exception {
		setupCall()
			.withServicePath("/" + MUNICIPALITY_ID + "/notes?partyId=e1cf339a-0ac7-4f3d-a6f6-9f687f9097e0") // partyId does not exist in DB.
			.withHttpMethod(HttpMethod.GET)
			.withExpectedResponseStatus(HttpStatus.OK)
			.withExpectedResponse("response.json")
			.sendRequestAndVerifyResponse();
